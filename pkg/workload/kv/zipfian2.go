...
func NewZipfGenerator(
	rng *rand.Rand, iMin, iMax uint64, theta float64,
) (*ZipfGenerator, error) {
	if iMin > iMax {
...
func (z *ZipfGenerator) Uint64(n int) []uint64 {
	z.zipfGenMu.mu.Lock()

	iMin := z.iMin
	resMap := make(map[uint64]struct{})
	...
	for i := 0; i < n; i++ {
		...
		for {
			...
			if uz < 1.0 {
				...
				num = iMin + uint64(int64(spread*math.Pow(z.zipfGenMu.eta*u-z.zipfGenMu.eta+1.0, z.alpha)))
			}
			if num == iMin {
				iMin++
			}
